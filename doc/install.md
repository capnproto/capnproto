...
<pre><code>curl -O <a href="https://capnproto.org/capnproto-c++-0.9.2.tar.gz">https://capnproto.org/capnproto-c++-0.9.2.tar.gz</a>
tar zxf capnproto-c++-0.9.2.tar.gz
cd capnproto-c++-0.9.2
./configure
make -j6 check
...
1. Download Cap'n Proto Win32 build:
   ...
   <pre><a href="https://capnproto.org/capnproto-c++-win32-0.9.2.zip">https://capnproto.org/capnproto-c++-win32-0.9.2.zip</a></pre>

2. Find `capnp.exe`, `capnpc-c++.exe`, and `capnpc-capnp.exe` under `capnproto-tools-win32-0.9.2` in
...
3. If your `.capnp` files will import any of the `.capnp` files provided by the core project, or
   ...
   directory `capnproto-c++-0.9.2/src` to the location of your choice, then make sure to pass the
...
3. Use CMake to generate Visual Studio project files under `capnproto-c++-0.9.2` in the zip file.
