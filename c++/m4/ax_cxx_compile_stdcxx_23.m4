...
#  Further Modified yet again for C++20 by Harris Hancock.
#  Further Modified yet again for C++23 by Mike Aizatsky.
# ============================================================================
#
# SYNOPSIS
#
#   AX_CXX_COMPILE_STDCXX_23([ext|noext])
#
# DESCRIPTION
#
#   Check for baseline language coverage in the compiler for the C++23
#   standard; if necessary, add switches to CXXFLAGS to enable support.
#   Errors out if no mode that supports C++23 baseline syntax can be found.
#   The argument, if specified, indicates whether you insist on an extended
#   mode (e.g. -std=gnu++23) or a strict conformance mode (e.g. -std=c++23).
...
m4_define([_AX_CXX_COMPILE_STDCXX_23_testbody], [[
  ...
  // C++20 stuff
  constinit const char* aConstantString = "abcd";

  // C++23 stuff
  struct DeducingThis {
    auto addRefToThis(this auto&& self) { return self; }
  };
]])
...
AC_DEFUN([AX_CXX_COMPILE_STDCXX_23], [dnl
  m4_if([$1], [], [],
        [$1], [ext], [],
        [$1], [noext], [],
        [m4_fatal([invalid argument `$1' to AX_CXX_COMPILE_STDCXX_23])])dnl
  AC_LANG_ASSERT([C++])dnl
  ac_success=no
  AC_CACHE_CHECK(whether $CXX supports C++23 features by default,
  aax_cv_cxx_compile_cxx23,
  [AC_COMPILE_IFELSE([AC_LANG_SOURCE([_AX_CXX_COMPILE_STDCXX_23_testbody])],
    [aax_cv_cxx_compile_cxx23=yes],
    [aax_cv_cxx_compile_cxx23=no])])
  if test x$aax_cv_cxx_compile_cxx23 = xyes; then
    ac_success=yes
  fi

  m4_if([$1], [noext], [], [dnl
  if test x$ac_success = xno; then
    for switch in -std=gnu++23 -std=gnu++2a; do
